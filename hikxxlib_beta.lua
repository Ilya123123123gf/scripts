...
channelList.Size = UDim2.new(1, 0, 0.7, 0)
channelList.Position = UDim2.new(0, 0, 0.3, 0)
channelList.BackgroundTransparency = 1
channelList.ScrollBarThickness = 4
channelList.Parent = leftPanel
...
-- Right panel: content for clicked channel
...
local function selectChannel(channelName)
    selectedChannel = channelName
    clearFrame(rightContent)

    local buttons = buttonsForChannel[channelName] or {}
...
function lib:CreateServer(name, icon)
    ...
    -- When server clicked, populate channels for this server
    btn.MouseButton1Click:Connect(function()
        clearFrame(channelList)
        channels[name] = channels[name] or {}

        for _, channelName in ipairs(channels[name]) do
            local chBtn = Instance.new("TextButton")
            chBtn.Size = UDim2.new(1, 0, 0, 30)
            chBtn.BackgroundColor3 = theme.BG
            chBtn.TextColor3 = theme.Text
            chBtn.Font = Enum.Font.Gotham
            chBtn.TextSize = 16
            chBtn.Text = channelName
            chBtn.Parent = channelList
            chBtn.AutoButtonColor = true

            Instance.new("UICorner", chBtn)
    ...
    return {
        AddChannel = function(channelName)
            channels[name] = channels[name] or {}
            table.insert(channels[name], channelName)
            buttonsForChannel[channelName] = {}
        end,

        AddButtonToChannel = function(channelName, btnName, callback)
            buttonsForChannel[channelName] = buttonsForChannel[channelName] or {}
            table.insert(buttonsForChannel[channelName], {name = btnName, callback = callback})
        end
    }
end
